...
Git has another mode for generating diffs called `word-diff`. 
```bash
git diff --word-diff a.tsx b.tsx
```
...
Github doesn't support this mode yet.
</Preview>

<Preview
  id="ai"
  label="AI"
  description="Using AI to analyze the output of `git diff --word-diff`."
  src="/previews/diff-viewer/ai"
>

### Using AI

If asking o3 to analyze the output of `git diff --word-diff`, it will identify lines that have changed more than 45% and need to be reformatted.
